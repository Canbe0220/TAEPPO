...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, case, env_paras, data_source='case'):
        ...
        feat_mas_batch[:, 1, :] = torch.sum(self.proc_times_batch, dim=-2).div(feat_mas_batch[:, 0, :] + 1e-9)
        ...
        proc_pair_batch[..., 1] = (self.proc_times_batch[batch_indices, start_opes] - self.min_proc).div(self.max_proc - self.min_proc + 1e-8)
    ...
    def step(self, actions):
        ...
        self.feat_mas_batch[self.batch_idxes, 1, :] = torch.sum(self.proc_times_batch[self.batch_idxes, ...], dim=-2) \
                                                      .div(self.feat_mas_batch[self.batch_idxes, 0, :] + 1e-9)
        self.feat_mas_batch[self.batch_idxes, 2, mas] = self.time[self.batch_idxes] + self.proc_times_batch[self.batch_idxes, opes, mas]    
        self.feat_mas_batch[self.batch_idxes, 3, :] = self.mask_ma_procing_batch[self.batch_idxes, :].float()
        ...
        self.proc_pair_batch[self.batch_idxes, jobs, :, 1] = (self.proc_times_batch[self.batch_idxes, next_opes, :]  - self.min_proc) / (self.max_proc - self.min_proc + 1e-8)
